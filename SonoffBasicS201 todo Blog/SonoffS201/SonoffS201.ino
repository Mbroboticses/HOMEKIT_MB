...
const char* host = "SonoffS201"; // nombre del entorno Sonoff1estado
...
void loop() {
  ...
  if (estado_pulsador==LOW)
  {
    ...
    {
      estado_luz=1;
      client.publish("SonoffS201","SwitchedOn");
      Serial.println("Switch On");
    }
    else
    {
      estado_luz=0; 
      client.publish("SonoffS201","SwitchedOff");
      Serial.println("Switch Off");
    }
    ...
    if (WiFi.status() == WL_CONNECTED) {
    if (!client.connected()) {
      if (client.connect("ESP8266: SonoffS201")) {
