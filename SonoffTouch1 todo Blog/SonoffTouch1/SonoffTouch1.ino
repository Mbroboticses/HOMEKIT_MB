...
const char* host = "SonoffTouch1"; // nombre del entorno Sonoff1estado
...
void loop() {
  ...
  if (estado_pulsador==LOW)
  {
    ...
    {
      estado_luz=1;
      client.publish("SonoffTouch1","SwitchedOn");
      Serial.println("Switch On");
    }
    else
    {
      estado_luz=0; 
      client.publish("SonoffTouch1","SwitchedOff");
      Serial.println("Switch Off");
    }
    ...
    if (WiFi.status() == WL_CONNECTED) {
    if (!client.connected()) {
      if (client.connect("ESP8266: SonoffTouch1")) {
